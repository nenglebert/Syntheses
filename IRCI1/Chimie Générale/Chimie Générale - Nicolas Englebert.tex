...
Les unités de $K$ dépendent de la réaction, mais elles sont facultatives. Cette constante dépend de la stœchiométrie.\\
...
\end{center}
Il y a trois facteurs qui influencent l'équilibre (\textbf{pas} la constante, sauf la 3 !)
\begin{enumerate}
...
\begin{description}
	...
	\item[Base] Substance qui produit des ions $OH^-$ en solution aqueuse
\end{description}
\ \\
...
Quand plusieurs bases sont mises ensemble, il va y avoir une "compétition" pour savoir laquelle mérite d'avoir un beau petit $H^+$.

\section{Force d'un acide}
Un acide fort se remarque par un $K_a$ très élevée et une dissociation quasi-complète.\\

Un \textit{amphotère} est une substance qui se comporte soit comme un acide, soit comme une base ; réaction d'auto-ionisation.
...
\section{Polyacides}
Il s'agit d'acides pouvant céder plus d'un proton. Bien souvent $K_{a1} >> K_{a2}$ : Ceci est du au faut qu'après avoir arraché un $H^+$, ça devient plus compliquer d'en arracher un deuxième (c'est donc un moins bon acide).\\
...
Un sel est un composé ionique impliquant donc la présence d'ions en solution. Ils peuvent se comporter comme des acides, comme des bases ou simplement être spectateurs. \\
...
Il s'agit d'une solution dont le pH varie très peu après addition d'une petite quantité de $H^+$ ou $OH^-$.\\
