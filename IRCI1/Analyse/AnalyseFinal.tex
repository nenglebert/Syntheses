...
\usepackage{lmodern}
\usepackage{amsthm}
\usepackage{verbatim} %Pour commentaires multilignes
...
\begin{document}
\setlength{\fboxsep}{3 mm } %Fait de jolies boites
...
\\
\subsubsection*{Axiome de Dedekind}
...
\subsection{Complétion de $\mathbb{R}$ par $\pm \infty$}
...
\newpage
\section{Limites}
\subsection{Limite infinie d'une fonction}
...
\subsection*{Calcul des limites de suites}
...
\subsection*{Croissance et décroissance de suite}
...
\subsubsection{o, O, ~ et comportement équivalent}
...
\subsubsection{Suite partielle, queue de suite}
...
\subsubsection{Limites sup et inf}
...
\subsubsection{Limites d'une fonction un un point réel}
...
\subsection{Continuité, discontinuité}
...
\subsubsection{Espèces de discontinuité}
...
\subsubsection{Fonction croissante et décroissante}
...
\subsubsection{Suprémum d'une fonction}
Le \textit{suprémum} de f : $sup\ f := sup{f(x) | x \in A}$

\subsubsection{Discontinuité des fonctions monotones}
...
\subsection{Ensemble de niveau, image fonction réciproque}
...
\subsubsection{Théorème de la valeur intermédiaire (TVI)}
...
\subsubsection*{Application du TVI}
...
\subsection{Image d'un compact par $f$ continue}
...
\subsubsection{Existence du maximum de f}
...
\subsubsection*{Dérivable implique continue}
...
\subsection{Règle de calcul de dérivées, différentielles}
\subsubsection{Dérivée d'une composée}
...
\subsection{Dérivée en un extrémant local}
...
\subsubsection{CN d'ordre 1 pour un extrémant local libre}
...
\subsubsection{Point critique, point suspect}
...
\subsubsection{Formule du reste de Lagrange}
...
\subsection{Croissance de f est signe de sa dérivée}
\subsubsection{Croissance et dérivée positive}
Si $f$ est dérivable sur ]a,b[ $\rightarrow$ $f$ est croissante sur ]a,b[ $\rightarrow f' \geq 0$ (sur ]a,b[, $\forall x \in ]a,b[$)\\
La démonstration est à connaître.

\subsubsection{Test de la première dérivée non nulle}
...
\subsection{Définition et approche qualitatives}
\subsection{EDO}
...
\subsubsection{Équations à variables séparées}
...
\subsubsection{ED à VS : Cas général}
...
\subsection{Équations différentielles linéaires (EDL)}
\subsubsection{Opérateurs différentiels linéaires}
\subsubsection{EDL d'ordre $n$}
...
\subsubsection{SGEnH = SGEH + SPEnH}
...
\subsubsection{Solution générale d'une EDL homogène d'ordre 1}
...
\subsubsection{Variation de la constante}
...
\subsubsection{Existence et unicité globale}
...
\subsection{Réduction du $2^e$ ordre au $1^{er}$}
Méthode pratique, \textit{cf. Analyse I, 6.9}.

\subsection{EDLH du $2^e$ ordre à coefficients constants}
EDLH est l'abréviation d'Equation Différentielle Linéaire Homogène de la forme :
$$y'' + a_1y' + a_0y = 0\ \ \ \ \ \ \ \ (E_h)$$
\subsubsection{L'espace vectoriel des solutions}
La dimension de l'espace vectoriel vaut l'ordre de $E_h$.

\subsubsection{Rappel pour l'ordre 1}
Monte, feignant !
...
\subsubsection{Racines réelles distinctes}
...
\subsubsection{Racines complexes conjuguées distinctes}
...
\subsubsection{Racines confondues}
...
\subsubsection{Parties réelles et imaginaires de solutions complexes}
...
\subsection{EDL non homogène du second ordre}
Il s'agit d'une équation du type : 
$$y'' + a_1y' + a_0y = b(x)\ \ \ \  (E_b)$$

\subsubsection{Méthode de la variation des constantes}
Comme énoncé ci-dessus, c'est principalement pratique : \textit{cf. TP 9}

\subsubsection{Quelques avantages de la linéarité}
...
\section{Fonctions à valeurs complexes et vectorielles}
Le chapitre a été passé. (Pour l'instant ?)

\newpage
\section{Fonctions de plusieurs variables}
\subsection{Exemples et représentations géométriques}
Il s'agit des 38 premières pages du syllabus, elles sont données à titre informatif.

\subsection{Limite et continuité locale}
\subsubsection{Limite d'une fonction en un point}
Définition :\\
...
\subsubsection{Continuité d'une fonction en un point (de son domaine)}
...
\subsubsection{Limites composantes par composantes}
...
\end{center}
\subsubsection{Limites restreintes}
...
\subsubsection{Discordances des limites partielles}
...
\subsubsection{Image continue d'un connexe}
...
\subsubsection{Image continue d'un compact et bornes atteintes}
...
\subsubsection{Gradient et matrice jacobienne}
...
\subsubsection{Dérivée directionnelle pour une fonction vectorielle}
...
\subsection{Différentielle $df(\vec{a}$) de $f$ en $\vec{a}$}
\subsubsection{La différentielle : $1^{ère}$ approche}
\textit{cf. Confond}

\subsubsection{Différentiabilité et différentielle : définitions}
...
\subsubsection{Différentielles et dérivées partielles}
...
\subsubsection*{Application aux TP}
...
\subsection{Règles de calculs et applications}
\subsubsection{Règles de calculs}
Ce sont les mêmes que pour les dérivées, mais en $\partial$ ; je ne les recopie donc pas ici.

\subsubsection{Différentielle d'une composée de fonction}
Si
\begin{center}
...
\subsubsection{Dérivations en cascades}
...
\subsubsection{Laplacien d'une fonction radiale}
...
\subsubsection{Laplacien en coordonnée polaires}
Sera rajouté après le TP 13.

\subsection{Applications géométriques de la différentielles}
\subsubsection{Hyperplans tangents au graphe d'une fonction scalaire}
...
\subsubsection{Gradient et plus grande dérivée directionnelle}
...
\subsubsection{Gradient et ensembles de niveau d'une fonction scalaire}
...
\subsection{Taylor : approche locale et majoration}
...
\subsubsection*{Corolaire 1 :Théorème majorant le taux d'accroissement}
Si $\vec{f}$ respecte les conditions du TAF (flemme) alors :
$$\frac{||\vec{f}(b) - \vec{f}(a)||}{|b -a|} \leq ||f'(c)||$$

\subsubsection{Développement de Taylor d'ordre $k$ de $f$ autour de $\vec{a}$}
...
\subsubsection{Minimum, minimant et consors}
...
\subsubsection{CN d'extrémant local libre}
\textbf{Proposition : }\\
...
\subsubsection{Extrémums globaux}
...
\subsubsection{Conditions du secon ordre d'extrémant local libre}
...
\subsubsection{Coefficient de dilatation d'une application linéaire}
Le coefficient d'une application linéaire $\vec{T}$ est $\sqrt{det(^tAA)}$ ou $A$ est la matrice de $\vec{T}$ (dans les bases canoniques).

\newpage
\section{Intégrales Riemannienne}
\subsection{Approche d'une intégrale simple par sommes finies (1D)}
\subsubsection{Intégrales et aires}
...
\end{center}
\subsubsection{Subdivision en escalier}
...
\subsubsection{Intégrale d'une fonction bornée}
...
\subsubsection{Convergence des sommes de Riemann}
...
\subsection{Propriétés de l'intégrale simple}
\subsubsection{Additivité relativement au domaine d'intégration}
...
\subsubsection{Intégrabilité versus domaine de discontinuité}
...
\subsubsection{Linéarité de l'intégration sur $[a,b]$} 
Je pense qu'avec Algèbre, c'est assez clair.

\subsubsection{Intégrabilité de composées, produits, valeur absolue}
...
\subsubsection{Croissance de l'intégration sur $[a,b]$}
...
\subsubsection{Théorème de la moyenne (du calcul intégral)}
...
\subsubsection{Intégrale nulle - Intégrande nulle ?}
...
\subsubsection{Continuité et dérivation d'intégrales définies}
...
\subsubsection{Règles de Leibniz}
...
\subsubsection{Théorème fondamental du CDI}
Si : 
\begin{enumerate}
...
\subsection{Formules de substitution et d'intégration par partie}
Seulement des applications en TP.

\subsection{Longueurs, aires, volumes par intégrales simples}
Juste vu le point \textit{9.4.4 La trompette de Torricelli} qui est à titre "informatif".

\subsection{Intégrales sur un pavé}
\subsubsection{Intuitions géométriques et pondérée}
En toute rigueur, on ne peut pas a priori savoir ce qu'est le volume, la "masse totale".

\subsubsection{Subdivision du domaine}
...
\subsubsection{Petites et grandes sommes de Darboux}
...
\subsubsection{Définition de l'intégrale de $f$ sur $P$}
...
\subsubsection{Sommes de Riemann et norme d'une subdivision}
...
\subsubsection{Intégrales triples, intégrales multiples}
$$\iiint_P f(\vec{x})d\vec{x}$$

\subsubsection{Intégrales multiples dans un domaine $nD$}
C'est pareil !
$$\iiint...\iiint$$ 

\subsection{Intégrales emboîtées}
\subsubsection{Deux intégrales emboitée}
La notation est la suivante : 
$$\int_c^d \int_a^b f(x, y) dx dx := \int_c^d A(y) dy$$

\subsubsection{Théorème de Fubini sur un rectangle}
...
\subsubsection{Cas ou Fubini ne s'applique pas}
...
\subsubsection{Théorème de Fubini sur un pavé $nD$}
Il y à dès lors $!n$ ordres d'intégration possibles.

\subsection{Intégrales sur une région mesurable}
\subsubsection{Mesure nulle selon Riemann}
...
\subsubsection{Intégrales sur des domaines inclus dans $\mathbb{R}^n$}
Les deux sections précédentes s'étendent de manière évidente à la dimension $n$.

\subsubsection{Contribution d'un ensemble de mesures nulles}
Si $g = f$ sur $G$ sauf une partie de $n-$mesure = 0, alors $g$ est intégrables et 
$$\int_G g = \int_G f$$

\subsection{Théorème de Fubini sur une région de $\mathbb{R}^n$}
\subsubsection{Région normales dans $\mathbb{R}^2$}
...
\subsubsection{Théorème de Fubini sur domaine normal 2D}
...
\subsubsection{Calcul d'aires planes : Fubini confirme}
...
\subsubsection{Applications de Fubini dans $\mathbb{R}^2$}
Méthode à suivre en TP, \textit{cf. page 71}.

\subsubsection{Théorème de Fubini par sections planes dans $\mathbb{R}^3$}
...
\subsubsection{Théorème de Fubini par sections rectilignes dans $\mathbb{R}^3$}
...
\subsection{Propriétés des intégrales multiples}
\subsubsection{Additivité de l'intégration d'une même fonction}
...
\subsubsection{Linéarité de l'intégration sur un même domaine}
...
\subsubsection{Croissance de l'opérateur $\int_G$}
Si $\forall \vec{x} \in G : f(\vec{x}) \leq \tilde{f}(\vec{x})$ alors
$$\int_G f \leq \int_G \tilde{f}$$

\subsubsection{Une norme sur $C^0$}
Pas vu en cours ! :)

\subsubsection{Théorème de la majoration d'intégrales}
...
\subsubsection{Valeur moyenne, théorème de la valeur moyenne}
...
\subsection{Changement de variables dans les intégrales multiples}
