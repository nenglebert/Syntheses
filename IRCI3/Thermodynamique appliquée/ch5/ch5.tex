...
\vec{c}.\vec{n}\ dS = -\sum\dot{m_s}\left(\frac{p}{\rho}\right)_e-\int_{S_s}
p\vec{b}_0.\vec{n}\ dS\\
...
\begin{equation}
\frac{d}{dt}\left(U+E_{cin}+E_{pot}\right)_F + \sum \dot{m_s}\left(u+\frac{c^2}{2}+gz\right)_s-\sum \dot{m}_e\left(\frac{p}{\rho}\left(u+\frac{c^2}{2}+gz\right)_e = \dot{Q_F}+\dot{W_O}
