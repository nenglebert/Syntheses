...
\section{Série de Fourier}
	...
	\exemple{Soit la fonction $x(t)$, nulle partout sauf dans l'intervalle $[-T_1,T_1]$, ayant une surface vallant 1 et périodique de période $T$ ($T_1<T$). Calculons ses coefficients de Fourier 
\begin{eqnarray}
...
que la transformée de Fourier du signal accoustique.
	...
	\exemple{
	\begin{eqnarray}
	X(i\omega) &=&\int_{-\infty}^{+\infty}\overbrace{x(t)}^{e^{-at}u(t)}e^{-i\omega t}\,dt=\int_{0}^{+\infty}e^{-(a+i\omega)t}\,dt\\
