...
\section{Intégrale d'une fonction complexe d'une valeur complexe}
    ...
    \begin{equation}
    \int_a^\pi \frac{1}{R}e^{-i\theta}Rie^{i\theta} = i\pi
    \end{equation}
    ...
    \begin{equation}
    \int_a^\pi \frac{1}{R}e^{i\theta}R(-i)e^{-i\theta} = -i\pi
    \end{equation}
    \end{enumerate}} 
...
de $\mathcal{C}\cup D$ (où $D$ est un domaine intérieur à $\mathcal{C}$) :
\begin{equation}
...
\end{equation}
je ne peux rien dire avec Cauchy-Gourmet, la fonction étant non analytique en 0. Mais 
