...
\section{Séries de Taylor et de Laurent}
	...
	Si $f$ est analytique dans le disque ouvert $D$ et que $|z-z_0| < R$, alors en tout point 
	$z$ du disque : 
	\begin{equation}
	...
	\begin{equation}
	e^z \approx 1+z+\frac{z^2}{2} + \dots + \frac{z^n}{n!} + \dots\ \ \ \ (|z|<\infty)
	\end{equation}
	...
	1}{(1-z)^3}, \dots, f^{(n)} = \frac{1}{(1-z)^{n+1}}$. Notre développement vaut alors :
	\begin{equation}
	...
	Alors :
	\begin{equation}
	f(z) = \sum_{n=0}^\infty a_n(z-z_0)^n + \sum_{n=2}^\infty \dfrac{b_n}{(z-z_0)^n}
	\end{equation}
	où
	\begin{eqnarray}
	...
	Secondement :
	\begin{equation}
	\frac{-1}{z-2} = -\frac{1}{2(\frac{z}{2}-1)} = \frac{1}{z(1-\frac{z}{2})} = \frac{1}{2}
	\sum_{n=0}^\infty\left(\frac{z}{2}\right)^n\ \ \ (|1/z|<2 \rightarrow |z|<2)
	\end{equation}
	Ainsi 
	\begin{equation}
	f(z) = \sum_{n=1}^\infty\frac{1}{z^n} + \sum_{n=0}^\infty\left(\frac{z}{2}\right)^n\ \ \
	(1<|z|<2)
	\end{equation}}
...
\section{Propriétés des séries de puissances}
	...
	z_1-z_0|$, le distance entre les deux points.}
	...
	\end{description}
	Ceci étant fait, il est précieux se savoir que si l'on a une série de puissance \textit{négative} qui converge, alors cette même série converge absolument dans la région extérieure 
	...
	Ceci explloite directement la définition de la continuité. Soit $S(z) = \sum_{n=0}^\infty 
	...
	\subsection{Unicité de la représentation par une série de puissance}
		...
		\exemple{Soit la fonction $f(z)=\sum\limits_0^{\infty}a_n(z-z_0)^n\quad|z-z_0|<2$. Amusons-nous à calculer $f(z)$ en $z_0$ ainsi que ses dérivées : 
		\begin{equation}
		f(z_0)=a_0
		\end{equation}
		\begin{equation}
	...
	\end{equation}
	\textbf{NICO A OUBLIÉ LA SUITE} -\_-
	}
